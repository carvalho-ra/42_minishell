...
/*   Updated: 2023/08/12 08:18:41 by cnascime         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
			...
			if (token->error_code)//Em caso de erro
			{
				if (token->error_code == 2)
					...
					printf("minishell: %s com erro: %s\n", \
						reference->cmd[0], strerror(token->error_code));
				return (-1);
			}
			//Se não tiver erro, abre o arquivo no modo leitura
			token->pipe[0] = open(reference->cmd[0], O_RDONLY);
	...
	while (token)
	{
		if (token->type == REDIRECT_OUT || token->type == APPEND)
		{
			...
			if (token->type == REDIRECT_OUT)
				token->pipe[1] = open(reference->cmd[0], \
										O_WRONLY | O_CREAT | O_TRUNC, 0644);
			else if (token->type == APPEND)
				token->pipe[1] = open(reference->cmd[0], \
										O_WRONLY | O_CREAT | O_APPEND, 0644);
			printf("\t\tnome do arquivo: %s, fd_out: %i\n", reference->cmd[0], token->pipe[1]);
			if (token->pipe[1] < 0)
			{
